...
module Api.Request.ProposeAssignments exposing
...
proposeAssignmentsApi : Maybe Float -> Maybe String -> String -> Api.Request (List Api.Data.ProposedAssignment)
proposeAssignmentsApi minConfidence_query customerId_query auth_token =
    Api.request
        "GET"
        "/api/v1/bookkeeping/propose-assignments"
        []
        [ ( "min_confidence", Maybe.map String.fromFloat minConfidence_query ), ( "customer_id", Maybe.map identity customerId_query ) ]
        []
        Nothing
        (Json.Decode.list Api.Data.proposedAssignmentDecoder)
